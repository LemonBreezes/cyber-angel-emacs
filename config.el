...
(unless (memq system-type '(cygwin windows-nt ms-dos))
  ...
  (setq doom-font (font-spec :family "Iosevka Term" :size 16) ; Iosevka Comfy
        doom-variable-pitch-font (font-spec :family "Iosevka Term" ; Iosevka Comfy Duo
                                            :size 16)
...
(when cae-init-tools-enabled-p
  ...
  (after! browse-url
    ...
    (cond ((getenv "WSL_DISTRO_NAME")
          ...
          ((or (executable-find "chromium-bin-browser")
  ...
  (use-package! avy
    ...
    (setq avy-timeout-seconds 0.4
          ...
          avy-keys (cae-keyboard-remap
                    '(?a ?s ?d ?f ?g
                      ?h ?j ?k ?l ?\;))
          avy-background (cae-display-graphic-p)
          avy-dispatch-alist
          (cae-keyboard-remap
