...
;;;###autoload
(defun cae-randomize-region (beg end)
  (interactive
   (if (use-region-p)
       (list (region-beginning) (region-end)) ; use the current region if it is active
     (list (point-min) (point-max)))) ; if no region is active, use the whole buffer
  ;; Do not shuffle the first line in dired-mode.
