...
(defun normal-escape ()
  ...
  (let ((event (read-event nil nil 0.15)))
    (if (not event)                      ; No event soon after, so just escape out...
        (progn
          (setq normal-escape-enabled t) ; Makes sure we re-enable escape
                                         ; checking Maybe this should
                                         ; conditionally do keyboard-escape-quit
                                         ; when in the minibuffer?
          (keyboard-quit))               ; Recent event happened right after, so just put it back in effect...
    ...
    (unless (boundp 'cae-keyboard--lispy-mode-map-backup)
      ...
      (defvar cae-lispy-hydra--command-column-alist
        '((special-lispy-right . "")
          ...
          (special-lispy-eval-and-insert . "Eval")
