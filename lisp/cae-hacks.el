...
it will output a message explaining why, remove the buffer from all perspectives, and allow it to be killed."
  :around #'persp-kill-buffer-query-function
  (let ((result (funcall orig-fn))
        buffer (current-buffer))
    (if result
        ;; Original function returned t, meaning the buffer can be killed normally
        result
      ;; Original function returned nil, meaning it's preventing the buffer from being killed
      (let ((buffer-name (buffer-name buffer))
            (reason ""))
        ;; Try to determine why the buffer wasn't killed
        (cond
         ;; Check if it's the last buffer in a perspective
         ((and (boundp 'persp-autokill-persp-when-removed-last-buffer)
               (not (eq 'kill persp-autokill-persp-when-removed-last-buffer))
               (let ((persp (get-current-persp)))
                 (and persp
                      (= 1 (length (safe-persp-buffers persp)))
                      (memq buffer (safe-persp-buffers persp)))))
          (setq reason "it's the last buffer in the current perspective"))

         ;; Check if it's a special buffer
         ((and buffer-name (string-match "\\`\\*.*\\*\\'" buffer-name))
          (setq reason "it's a special buffer"))

         ;; Default case
         (t
          (setq reason "of an unknown reason in persp-kill-buffer-query-function")))

        ;; Output explanation
        (message "Buffer '%s' was not killed because %s. Forcing kill..."
                 buffer-name reason)
