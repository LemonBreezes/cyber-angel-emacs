...
(eval
 `(defhydra +mpc-hydra (:color pink :hint nil)
    ...
    ("<" mpc-prev "Previous song" :column "Player")
    (">" mpc-next "Next song" :column "Player")
    ...
    ,@(when (modulep! :editor evil)
        '(("r" mpc-toggle-repeat "Toggle repeat" :column "Toggle")
          ...
          ("p" mpc-playlist "Show playlist" :column "View")
          ("a" mpc-playlist-add "Add to playlist" :column "Player")
          ("x" mpc-play-at-point "Play at point" :column "Player")))
    ("p" mpc-pause "Pause" :column "Player")
    ("g" mpc-seek-current "Seek current" :column "Player")
    ("o" mpc-goto-playing-song "Goto playing song" :column ""))
 t)
